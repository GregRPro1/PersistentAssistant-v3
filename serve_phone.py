...
def health():
    return jsonify({"ok":True,"has_token":bool(load_token())})
...
def approve():
    ...
    now = int(time.time())
    if not (now-TS_SKEW <= ts <= now+TS_SKEW):
        return jsonify({"error":"timestamp_out_of_range","now":now,"ts":ts,"skew":TS_SKEW}), 400
    if len(nonce) < 8:
