<# 
PA-361 TUNNEL AUTO (FIXPERMS2)
- Robust cloudflared detection (PATH, Program Files, LocalAppData, tools\bin, where.exe)
- If missing, try winget -> choco -> direct download (falls back to %TEMP% if tools\bin is locked)
...
function Find-Cloudflared {
  ...
  $cands = @()
  $cands += (Join-Path (Join-Path $PSScriptRoot '..\..\tools\bin') 'cloudflared.exe')
  if ($env:ProgramFiles)      { $cands += (Join-Path $env:ProgramFiles      'Cloudflare\Cloudflared\cloudflared.exe') }
  if ($env:ProgramFiles(x86)) { $cands += (Join-Path ${env:ProgramFiles(x86)} 'Cloudflare\Cloudflared\cloudflared.exe') }
  if ($env:LOCALAPPDATA) {
...
# Commit (no push)
try { git add -A; git commit -m "PA-361 TUNNEL_AUTO_FIXPERMS2: robust detection/install + tunnel_url.txt" | Out-Null } catch {}
