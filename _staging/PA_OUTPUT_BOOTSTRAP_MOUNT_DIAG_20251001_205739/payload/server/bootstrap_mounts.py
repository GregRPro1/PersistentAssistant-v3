...
def _mount(app):
    ...
    try:
        ...
        if hasattr(wd_api, "bp"):
            ...
            _log("mounted", blueprint="watchdog_api", url_prefix=getattr(wd_api.bp, "url_prefix", None))
        else:
    ...
    try:
        ...
        if hasattr(wd_ui, "bp"):
            ...
            _log("mounted", blueprint="watchdog_ui", url_prefix=getattr(wd_ui.bp, "url_prefix", None))
        else:
    ...
    try:
        ...
        _log("diag_endpoint_ready", path="/__wd_diag")
    except Exception as e:
...
def main():
    ...
    app = _import_app()
    _mount(app)
    _log("running_app", host=host, port=port, debug=debug)
