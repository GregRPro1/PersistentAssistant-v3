    ...
    if app is None:
        try:
            ...
            for _, name, _ in pkgutil.iter_modules(server.__path__):
                ...
                if hasattr(m, "app"):
                    app = getattr(m, "app")
                    _log("found_app", source=f"server.{name}")
                    break
        except Exception as e:
...
def _mount(app):
    # watchdog API (preferred)
    ok_api = False
    try:
        wd_api = importlib.import_module("server.watchdog_api")
        if hasattr(wd_api, "bp"):
            app.register_blueprint(wd_api.bp); ok_api = True
            _log("mounted", blueprint="watchdog_api", prefix=getattr(wd_api.bp, "url_prefix", None))
    except Exception as e:
        _log("mount_error", module="server.watchdog_api", error=str(e), tb=traceback.format_exc())
    # fallback API
    if not ok_api:
        try:
            wd_api = importlib.import_module("server.watchdog_api_fallback")
            if hasattr(wd_api, "bp"):
                app.register_blueprint(wd_api.bp); ok_api = True
                _log("mounted_fallback", blueprint="watchdog_api_fallback", prefix=getattr(wd_api.bp, "url_prefix", None))
        except Exception as e:
            _log("mount_error", module="server.watchdog_api_fallback", error=str(e), tb=traceback.format_exc())

    # watchdog UI (preferred)
    ok_ui = False
    try:
        wd_ui = importlib.import_module("server.watchdog_ui")
        if hasattr(wd_ui, "bp"):
            app.register_blueprint(wd_ui.bp); ok_ui = True
            _log("mounted", blueprint="watchdog_ui", prefix=getattr(wd_ui.bp, "url_prefix", None))
    except Exception as e:
        _log("mount_error", module="server.watchdog_ui", error=str(e), tb=traceback.format_exc())
    # fallback UI
    if not ok_ui:
        try:
            wd_ui = importlib.import_module("server.watchdog_ui_fallback")
            if hasattr(wd_ui, "bp"):
                app.register_blueprint(wd_ui.bp); ok_ui = True
                _log("mounted_fallback", blueprint="watchdog_ui_fallback", prefix=getattr(wd_ui.bp, "url_prefix", None))
        except Exception as e:
            _log("mount_error", module="server.watchdog_ui_fallback", error=str(e), tb=traceback.format_exc())
    ...
    try:
        ...
        @app.route("/__runner")
        def __runner():
            return {"runner":"server.bootstrap_mounts"}, 200
        _log("diag_ready", paths=["/__wd_diag","/__runner"])
    except Exception as e:
