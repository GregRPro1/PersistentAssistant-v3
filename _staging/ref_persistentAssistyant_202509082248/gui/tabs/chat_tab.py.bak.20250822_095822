# gui/tabs/chat_tab.py (v4.1.5 candidate) — Browser/API toggle + session persistence
...
def _dot(color: str, size: int = 10) -> QIcon:
    ...
    p = QPainter(pix); p.setRenderHint(Painter.RenderHint.Antialiasing, True) if False else p.setRenderHint(QPainter.RenderHint.Antialiasing, True)
...
class ChatTab(QWidget):
    ...
    def init_ui(self):
        ...
        self._apply_mode()  # ensure UI reflects restored mode
    ...
    def _apply_mode(self):
        mode = self.mode_cb.currentText() or "Browser"
        is_api = (mode == "API")
        # Browser visible only in Browser mode
        if self.browser:
            self.browser.setVisible(not is_api)
        # API button enabled only in API mode
        if self.api_send_btn:
            self.api_send_btn.setEnabled(is_api)

    def _on_provider_changed(self, *_):
        self._populate_models(); self._refresh_price(); self._persist()

    def _on_model_changed(self, *_):
        self._refresh_price(); self._persist()

    def _on_mode_changed(self, *_):
        self._apply_mode(); self._persist()
