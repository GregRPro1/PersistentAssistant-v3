import os, sys, imaplib, email, time, shutil, subprocess
...
def save_attachment(part, out_dir: Path, logf: Path):
    ...
    i=1
    while p.exists():
        p = out_dir / f"{p.stem}_{i}{p.suffix}"; i+=1
    p.write_bytes(data)
    log(logf, f"saved {p}")
    return p

def process_local(cfg, repo: Path, logf: Path):
    inbox = repo / cfg.get('save_dir','_inbox')
    processed = repo / cfg.get('processed_dir','_inbox/processed')
    inbox.mkdir(parents=True, exist_ok=True); processed.mkdir(parents=True, exist_ok=True)
    zips = sorted(inbox.glob("*.zip"))
    if not zips:
        log(logf, "no local zips to process"); return 0
    for zp in zips:
        ...
        if os.environ.get('PA_DRY_RUN')=='1':
            ...
            if ps1.exists():
                subprocess.call(['pwsh','-File', str(ps1)], cwd=repo)
            else:
                subprocess.call(['pwsh','-File', str(repo/'apply_packs_only.ps1'), '-ZipPath', str(zp)], cwd=repo)
        dest = processed / zp.name
        i=1
        while dest.exists(): dest = processed / f"{dest.stem}_{i}{dest.suffix}"; i+=1
        import shutil; shutil.move(str(zp), str(dest))
        log(logf, f"moved to {dest}")
    return 0

def poll_imap(cfg, repo: Path, logf: Path):
    host = cfg.get('host'); user=cfg.get('username'); pwd=cfg.get('password')
    if not host or not user or not pwd:
        log(logf, "IMAP config incomplete; skipping"); return 0
    port = int(cfg.get('port', 993))
    folder = cfg.get('folder','INBOX')
    flag = cfg.get('subject_flag','[PA] APPLY')
    allow_from = set([s.lower() for s in cfg.get('allow_from', [])])
    max_mb = int(cfg.get('max_attachment_mb', 50))
    save_dir = repo / cfg.get('save_dir','_inbox')
    processed_dir = repo / cfg.get('processed_dir','_inbox/processed')
    ...
    for num in data[0].split():
        ...
        subj = msg.get('Subject',''); frm = email.utils.parseaddr(msg.get('From',''))[1].lower()
        if allow_from and frm not in allow_from:
            log(logf, f"skip email from {frm}")
            continue
        if flag and flag not in subj:
            log(logf, f"skip email subj '{subj}' (missing flag)")
            continue
        saved = []
        for part in msg.walk():
            if part.get_content_disposition()=='attachment':
                ...
                if size > max_mb:
                    log(logf, f"skip {fn}: {size:.1f}MB > limit")
                    continue
                p = save_attachment(part, save_dir, logf)
                if p: saved.append(p)
        M.store(num, '+FLAGS', '\\Seen')
        if saved:
            log(logf, f"saved {len(saved)} zip(s) from {frm} subj='{subj}'")
    M.close(); M.logout()
...
def main():
    ...
    repo = resolve_repo_root(a.repo_root)
    if not repo:
    ...
    if not cfg.get('enabled'):
        print('email watcher disabled; exiting 0')
        return 0
    try:
        return poll_imap(cfg, repo, logf)
