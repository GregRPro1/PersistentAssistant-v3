...
if ($cf -and -not $Force) { Write-Host "cloudflared already present: $($cf.Source)"; cloudflared --version; exit 0 }
...
else { Write-Warning "No choco/winget. Install manually from Cloudflare docs"; exit 1 }
$cf2 = Get-Command cloudflared -ErrorAction SilentlyContinue; if (-not $cf2){ throw "cloudflared not found after install" }
cloudflared --version
