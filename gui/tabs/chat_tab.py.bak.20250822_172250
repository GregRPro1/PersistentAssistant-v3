# gui/tabs/chat_tab.py (v4.4 candidate) — robust error surfacing
...
class ChatTab(QWidget):
    ...
    def on_send_api_clicked(self):
        ...
        from tools.interaction_logger import log_interaction, log_error
        from tools.ai_error_mapper import summarize, log_error as log_errfile
        ...
        try:
            ...
            summary, detail, meta = summarize(e)
            # file logs
            try: log_errfile(summary, detail, meta)
            except Exception: pass
            # YAML error record
            try: err_path = log_error(text, prov, model or "", summary, detail, meta, ui_source="ChatTab")
            except Exception: err_path = None
            # concise popup
            extra = f"\n\n[Logged: {err_path}]" if err_path else ""
            QMessageBox.critical(self, "AI Error", summary + extra)
