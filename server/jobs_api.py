...
_thr = threading.Thread(target=_runner, daemon=True); _thr.start()

def _submit(kind:str, args:dict):
    job = Job(kind, args)
    with _lock: _jobs[job.id] = job
    job.save(); _q.put(job); return job

@bp.post(mount_path + '/apply')
def jobs_apply():
