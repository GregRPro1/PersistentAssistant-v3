...
function Start-Cmd([string]$key, [string]$cmdline){
  ...
  try {
    ...
    $null = $p.Start()
    $procs[$key] = $p
    Write-Log ".\tmp\logs\watchdog.log" "info" ("Started {0}: {1}" -f $key, $cmdline)
  } catch {
    Write-Log ".\tmp\logs\watchdog.log" "error" ("Start-Cmd({0}) failed: {1}" -f $key, $_.Exception.Message)
  }
}
...
function Stop-Tracker {
  try {
    ...
    Write-Log ".\tmp\logs\watchdog.log" "info" "Tracker processes stopped"
  } catch {
    Write-Log ".\tmp\logs\watchdog.log" "error" ("Stop-Tracker failed: {0}" -f $_.Exception.Message)
  }
}
...
function Snapshot(){
  ...
  if ($cfg.tunnel.enabled -and (Test-Path $cfg.tunnel.url_file)) {
    ...
    if (Is-GoodUrl $raw) { $tunnelUrl = $raw } else { if ($raw) { Write-Log ".\tmp\logs\watchdog.log" "warn" ("Ignoring invalid tunnel URL in file: '{0}'" -f $raw) } }
  }

  $lanIp = Get-LanIp
  $phoneLan = if ($lanIp) { ("http://{0}:{1}" -f $lanIp, $webPort) } else { "" }
  ...
  $st = @{
    ...
    watcher = @{ on = $watchHb }
    tunnel = @{ url = $tunnelUrl; ok = ([bool]$tunnelUrl) }
  ...
  try {
    ...
    foreach ($r in $reqs) {
      try {
        ...
        Write-Log ".\tmp\logs\watchdog.log" "error" ("Request handling failed for {0}: {1}" -f $r.Name, $_.Exception.Message)
      } finally {
        Remove-Item $r.FullName -Force -ErrorAction SilentlyContinue
      }
    }
  } catch {
    Write-Log ".\tmp\logs\watchdog.log" "error" ("Watchdog loop error: {0}" -f $_.Exception.Message)
  }
