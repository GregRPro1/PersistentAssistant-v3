...
$ops = Read-Json ".\reports\ops\ops_status.json"; $cfg = Read-Json ".\pal\config\pal_watchdog.json"; $port = 8787; if ($cfg -and $cfg.web -and $cfg.web.port) { $port = [int]$cfg.web.port }
$url = $null; if ($ops -and $ops.phone) { $url = $ops.phone.url; if (-not $url) { $url = $ops.phone.lan } }
if (-not $url) { $lan = Get-LanIp; if ($lan) { $url = "http://$($lan):$($port)" } elseif ($cfg -and $cfg.web -and $cfg.web.host) { $url = "http://$($cfg.web.host):$($port)" } }
