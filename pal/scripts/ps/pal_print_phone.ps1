# Print Phone URL (with fallback computation)
...
$ops = Read-Json ".\reports\ops\ops_status.json"; $cfg = Read-Json ".\pal\config\pal_watchdog.json"
$port = 8787; if ($cfg -and $cfg.web -and $cfg.web.port) { $port = [int]$cfg.web.port }
$url = $null
