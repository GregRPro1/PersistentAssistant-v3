...
class PalTracker(QWidget):
    ...
    def update_status_bar_ops(self):
        ...
        # Phone/Tunnel + fallback
        phone_url = d.get("phone",{}).get("url","") or d.get("phone",{}).get("lan","")
        if not phone_url:
            # fallback compute from LAN + configured port
            cfg = load_json(CFG_PATH)
            port = (cfg.get("web",{}) or {}).get("port", 8787)
    ...
    def change_status_selected(self, new_status: str):
        # no-op here; your main tracker already implements persistence in other pack
        pass
