#!/usr/bin/env python3
# PyQt6 tracker with window geometry persistence
...
def tcp_ping(host, port, timeout=3.0):
    import socket as s
    try:
        with s.create_connection((host, port), timeout=timeout): return True
...
class PalTracker(QWidget):
    def __init__(self, plan_path: Path):
        ...
        left=QWidget(); import PyQt6.QtWidgets as W; l=W.QVBoxLayout(left); l.setContentsMargins(8,8,8,4); l.setSpacing(6); l.addWidget(self.goals_label); l.addWidget(self.goals_list,1); l.addWidget(self.tree,3)
        ...
        self.status=QStatusBar(); self.status.setSizeGripEnabled(False); self.status.setStyleSheet("QStatusBar { background:#242424; color:#e1e1e1; padding:0 6px; } QStatusBar::item { border: 0px; }"); self.status.setFixedHeight(24)
    ...
    def restore_ui_state(self):
        try:
            ...
            if p.exists():
                ...
                if isinstance(g,list) and len(g)==4: self.setGeometry(int(g[0]),int(g[1]),int(g[2]),int(g[3]))
                if isinstance(sp,list) and len(sp)>=2: self.splitter.setSizes([int(sp[0]),int(sp[1])])
        except Exception: pass
    def save_ui_state(self):
        try:
            UI_CONFIG_PATH.parent.mkdir(parents=True, exist_ok=True)
            r=self.geometry(); cfg={"geometry":[int(r.x()),int(r.y()),int(r.width()),int(r.height())], "splitter": self.splitter.sizes()}
    ...
    def on_ping(self, ok: bool): self.ping_ok=ok; self.update_status_bar()
