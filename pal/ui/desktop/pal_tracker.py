#!/usr/bin/env python3
import sys, time, json, webbrowser, socket
...
CFG_PATH = Path("pal/config/pal_watchdog.json")
...
class SettingsPreserver:
    def __init__(self): self.cfg = load_json(UI_CONFIG_PATH)
    def geometry(self): return self.cfg.get("geometry")
    def set_geometry(self, rect):
        self.cfg["geometry"] = [int(rect.x()), int(rect.y()), int(rect.width()), int(rect.height())]
        save_json(UI_CONFIG_PATH, self.cfg)

class PalTracker(QWidget):
    def __init__(self, plan_path: Path):
        super().__init__(); self.plan_path=plan_path; self.plan={}; self.prefs=SettingsPreserver()
        ...
        self.saveTimer = QTimer(self); self.saveTimer.timeout.connect(lambda: self.prefs.set_geometry(self.geometry())); self.saveTimer.start(5000)

        self.refresh(initial=True)

    # --- Data helpers ---
    def pick_plan_path(self) -> Path:
    ...
    def load_plan(self):
        p = self.plan_path if self.plan_path.exists() else self.pick_plan_path()
    ...
    def color_for_status(self,s):
        ...
        return COLORS.get(s or "todo", COLORS["todo"])
    ...
    def update_status_bar_ops(self):
        d = load_json(OPS_STATUS) if OPS_STATUS.exists() else {}
        # Port
        p_ok = (d.get("web",{}) or {}).get("port_ok", False); port = (d.get("web",{}) or {}).get("port", 8787)
        ...
        # Health
        h_ok = (d.get("web",{}) or {}).get("health_ok", False)
        ...
        phone_url = phone.get("url") or phone.get("lan") or ""
        if not phone_url:
            cfg = load_json(CFG_PATH); port = (cfg.get("web",{}) or {}).get("port", 8787)
            ip = private_ip_fallback()
            if ip: phone_url = f"http://{ip}:{port}"
        ...
        # Tunnel
        tun_url = (d.get("tunnel",{}) or {}).get("url",""); t_ok = (d.get("tunnel",{}) or {}).get("ok", False)
        ...
        # Watcher
        w_on = (d.get("watcher",{}) or {}).get("on", False)
    ...
    def refresh(self, initial=False):
        ...
        self.prefs.set_geometry(self.geometry())
        # update
        p = self.plan_path if self.plan_path.exists() else self.pick_plan_path()
        if p.exists():
            try:
                import yaml; self.plan = yaml.safe_load(p.read_text(encoding="utf-8")) or {}
            except Exception: self.plan = {}
        else:
            self.plan = {}
        self.rebuild_tree(); self.update_status_bar()

    def closeEvent(self, e):
        self.prefs.set_geometry(self.geometry()); return super().closeEvent(e)
