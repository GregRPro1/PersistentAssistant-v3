# gui/tabs/chat_tab.py (v4.1.3d candidate) — traffic-light models + preserved API flow
...
def _status_to_icon(status: str) -> QIcon:
    s = (status or "").lower()
    if s == "success": return _dot("#2ecc71")   # green
    if s == "error":   return _dot("#e74c3c")   # red
    if s in ("disabled","skipped"): return _dot("#bdc3c7")  # grey
...
class ChatTab(QWidget):
    """
    Embedded Chat + model controls.
    - Preserves original API send flow (ensure_api_send_button, on_send_api_clicked).
    - Adds provider/model dropdowns with traffic-light probe status.
    - Adds pricing label, Browser/API toggle, and Ping button.
    """
    def __init__(self, parent=None):
        super().__init__(parent)
        self._models = None
        self._status = None
        self.api_send_btn = None  # for ensure_api_send_button compatibility
        self.init_ui()
    ...
    def init_ui(self):
        ...
        # API send button (also used by ensure_api_send_button)
        ...
        self.api_send_btn.clicked.connect(self.on_send_api_clicked)  # preserve handler
    ...
    def _on_provider_changed(self, *_): self._populate_models()
    def _on_model_changed(self, *_):    self._refresh_price()
    def _on_mode_changed(self, *_):     pass  # future: toggle flows
    ...
    def _on_ping_model(self):
        ...
        # Triggers global probe; status/icons update on next app start or reload
        try:
    ...
    def on_send_api_clicked(self):
        "Minimal API send path (preserved)."
        try:
            from core.ai_client import AIClient
        except Exception as e:
            QMessageBox.critical(self, "AI Error", f"AIClient import failed: {e}"); return
