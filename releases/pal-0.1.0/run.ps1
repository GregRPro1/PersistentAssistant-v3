...
# Ensure we run from the release folder
Set-Location -Path $PSScriptRoot

# Drop a tiny stdlib HTTP server (serves /healthz)
$serverPy = @"
...
Write-Host "Starting PAL placeholder on http://$Host`:$Port ..."
& $python $pyPath $Host $Port "0.1.0"
